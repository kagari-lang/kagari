...
The runtime ABI identity is `kagari-runtime-abi-v7`; the runtime-helper ABI is
v4. Previous ABI artifacts are rejected even when requested by the caller: v5
lacks shared mutation accounting; v6 lacks prepared path commits and quarantine.
The helper ABI preserves commit EngineFault status independently of resource/trap status.
